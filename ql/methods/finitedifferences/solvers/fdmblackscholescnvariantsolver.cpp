...
namespace QuantLib {
    ...
    FdmBlackScholesCnVariantSolver::FdmBlackScholesCnVariantSolver(
    ...
    void FdmBlackScholesCnVariantSolver::performCalculations() const {
        const ext::shared_ptr<FdmBlackScholesCnVariantOp> op(
            ext::make_shared<FdmBlackScholesCnVariantOp>(
    ...
    Real FdmBlackScholesCnVariantSolver::valueAt(Real s) const {
        calculate();
        return solver_->interpolateAt(std::log(s));
    }

    Real FdmBlackScholesCnVariantSolver::deltaAt(Real s) const {
        calculate();
        return solver_->derivativeX(std::log(s))/s;
    }

    Real FdmBlackScholesCnVariantSolver::gammaAt(Real s) const {
        calculate();
        return (solver_->derivativeXX(std::log(s))
                -solver_->derivativeX(std::log(s)))/(s*s);
    }

    Real FdmBlackScholesCnVariantSolver::thetaAt(Real s) const {
