...
namespace QuantLib {
    ...
    FdmBlackScholesExpFitSolver::FdmBlackScholesExpFitSolver(
    ...
    void FdmBlackScholesExpFitSolver::performCalculations() const {
        const ext::shared_ptr<FdmBlackScholesExpFitOp> op(
            ext::make_shared<FdmBlackScholesExpFitOp>(
    ...
    Real FdmBlackScholesExpFitSolver::valueAt(Real s) const {
        calculate();
        return solver_->interpolateAt(std::log(s));
    }

    Real FdmBlackScholesExpFitSolver::deltaAt(Real s) const {
        calculate();
        return solver_->derivativeX(std::log(s))/s;
    }

    Real FdmBlackScholesExpFitSolver::gammaAt(Real s) const {
        calculate();
        return (solver_->derivativeXX(std::log(s))
                -solver_->derivativeX(std::log(s)))/(s*s);
    }

    Real FdmBlackScholesExpFitSolver::thetaAt(Real s) const {
