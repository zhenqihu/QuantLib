/* -*- mode: c++; tab-width: 4; indent-tabs-mode: nil; c-basic-offset: 4 -*- */

/*
 Copyright (C) 2008 Andreas Gaida
 Copyright (C) 2008, 2009 Ralph Schreyer
 Copyright (C) 2008, 2009 Klaus Spanderen

 This file is part of QuantLib, a free-software/open-source library
 for financial quantitative analysts and developers - http://quantlib.org/

 QuantLib is free software: you can redistribute it and/or modify it
 under the terms of the QuantLib license.  You should have received a
 copy of the license along with this program; if not, please email
 <quantlib-dev@lists.sf.net>. The license is also available online at
 <http://quantlib.org/license.shtml>.

 This program is distributed in the hope that it will be useful, but WITHOUT
 ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 FOR A PARTICULAR PURPOSE.  See the license for more details.
*/

#include <ql/instruments/payoffs.hpp>
#include <ql/math/functional.hpp>
#include <ql/methods/finitedifferences/meshers/fdmmesher.hpp>
#include <ql/methods/finitedifferences/operators/fdmblackscholesexpfitop.hpp>
#include <ql/methods/finitedifferences/operators/fdmlinearoplayout.hpp>
#include <ql/methods/finitedifferences/operators/secondderivativeop.hpp>
#include <utility>

namespace QuantLib
{

    namespace {
        Real fittingFactor(Real mu, Real sigma, Real h) {
            QL_REQUIRE(h > 0.0, "invalid mesh size");

            // mu -> 0, rho -> sigma
            if (std::fabs(mu) < 1e-12)
                return sigma;

            // sigma -> 0, rho -> |mu| * h / 2
            if (std::fabs(sigma) < 1e-16)
                return 0.5 * std::fabs(mu) * h;

            const Real z = mu * h / (2.0 * sigma);

            // z -> 0, x cothx -> 1 + z^2 / 3 + O(z^4)
            if (std::fabs(z) < 1e-6)
                return sigma * (1.0 + z * z / 3.0);

            // coth(x) = 1 / tanh(x)
            return sigma * z / std::tanh(z);
        }
    }

    FdmBlackScholesExpFitOp::FdmBlackScholesExpFitOp(
        const ext::shared_ptr<FdmMesher> &mesher,
        const ext::shared_ptr<GeneralizedBlackScholesProcess> &bsProcess,
        Real strike,
        bool localVol,
        Real illegalLocalVolOverwrite,
        Size direction,
        ext::shared_ptr<FdmQuantoHelper> quantoHelper)
        : mesher_(mesher), rTS_(bsProcess->riskFreeRate().currentLink()),
          qTS_(bsProcess->dividendYield().currentLink()),
          volTS_(bsProcess->blackVolatility().currentLink()),
          localVol_((localVol) ? bsProcess->localVolatility().currentLink() : ext::shared_ptr<LocalVolTermStructure>()),
          x_((localVol) ? Array(Exp(mesher->locations(direction))) : Array()),
          dxMap_(FirstDerivativeOp(direction, mesher)), dxxMap_(SecondDerivativeOp(direction, mesher)),
          mapT_(direction, mesher), strike_(strike),
          illegalLocalVolOverwrite_(illegalLocalVolOverwrite), direction_(direction),
          quantoHelper_(std::move(quantoHelper)) {}

    void FdmBlackScholesExpFitOp::setTime(Time t1, Time t2)
    {
        const Rate r = rTS_->forwardRate(t1, t2, Continuous).rate();
        const Rate q = qTS_->forwardRate(t1, t2, Continuous).rate();

        const ext::shared_ptr<FdmLinearOpLayout> layout = mesher_->layout();
        const FdmLinearOpIterator endIter = layout->end();

        // calculate the mesh size，similar logic to SecondDerivativeOP
        Array h(layout->size());
        for (FdmLinearOpIterator iter = layout->begin(); iter != endIter; ++iter)
        {
            const Size i = iter.index();
            const Real hm = mesher_->dminus(iter, direction_);
            const Real hp = mesher_->dplus(iter, direction_);
            const Size co = iter.coordinates()[direction_];

            if (co == 0)
            {
                h[i] = hp;
            }
            else if (co == layout->dim()[direction_] - 1)
            {
                h[i] = hm;
            }
            else
            {
                h[i] = 0.5 * (hm + hp);
            }
        }

        Array variance(layout->size());
        if (localVol_ != nullptr)
        {
            for (FdmLinearOpIterator iter = layout->begin();
                 iter != endIter; ++iter)
            {
                const Size i = iter.index();

                if (illegalLocalVolOverwrite_ < 0.0)
                {
                    variance[i] = square<Real>()(
                        localVol_->localVol(0.5 * (t1 + t2), x_[i], true));
                }
                else
                {
                    try
                    {
                        variance[i] = square<Real>()(
                            localVol_->localVol(0.5 * (t1 + t2), x_[i], true));
                    }
                    catch (Error &)
                    {
                        variance[i] = square<Real>()(illegalLocalVolOverwrite_);
                    }
                }
            }
        }
        else
        {
            const Real v = volTS_->blackForwardVariance(t1, t2, strike_) / (t2 - t1);
            variance = Array(layout->size(), v);
        }

        Array diffusion = 0.5 * variance;
        Array drift = r - q - diffusion;

        if (quantoHelper_ != nullptr)
        {
            drift -= quantoHelper_->quantoAdjustment(Sqrt(variance), t1, t2);
        }

        Array rho(layout->size());
        for (Size i = 0; i < rho.size(); ++i)
        {
            rho[i] = fittingFactor(drift[i], diffusion[i], h[i]);
        }

        mapT_.axpyb(drift, dxMap_, dxxMap_.mult(rho), Array(1, -r));
    }

    Size FdmBlackScholesExpFitOp::size() const { return 1U; }

    Disposable<Array> FdmBlackScholesExpFitOp::apply(const Array &u) const
    {
        return mapT_.apply(u);
    }

    Disposable<Array> FdmBlackScholesExpFitOp::apply_direction(Size direction,
                                                               const Array &r) const
    {
        if (direction == direction_)
            return mapT_.apply(r);
        else
        {
            Array retVal(r.size(), 0.0);
            return retVal;
        }
    }

    Disposable<Array> FdmBlackScholesExpFitOp::apply_mixed(const Array &r) const
    {
        Array retVal(r.size(), 0.0);
        return retVal;
    }

    Disposable<Array> FdmBlackScholesExpFitOp::solve_splitting(Size direction,
                                                               const Array &r, Real dt) const
    {
        if (direction == direction_)
            return mapT_.solve_splitting(r, dt, 1.0);
        else
        {
            Array retVal(r);
            return retVal;
        }
    }

    Disposable<Array> FdmBlackScholesExpFitOp::preconditioner(const Array &r,
                                                              Real dt) const
    {
        return solve_splitting(direction_, r, dt);
    }

    Disposable<std::vector<SparseMatrix>>
    FdmBlackScholesExpFitOp::toMatrixDecomp() const
    {
        std::vector<SparseMatrix> retVal(1, mapT_.toMatrix());
        return retVal;
    }

}
